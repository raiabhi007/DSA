...
class Solution{
    public Tree cloneTree(Tree tree){
       ...
       Tree newTree = clone(tree,map);
       copyRandom(tree,map);
       return newTree;
     }
     public Tree clone(Tree tree,HashMap<Tree,Tree> map){
         if(tree==null)
         return null;
         Tree newNode = new Tree(tree.data);
         map.put(tree,newNode);
         newNode.left = clone(tree.left,map);
         newNode.right = clone(tree.right,map);
         return newNode;
     }
     public void copyRandom(Tree tree,HashMap<Tree, Tree> map){
        if (tree == null)
            return;
